...
- 1ff6953: Add Cognite instance intrinsics to typed query filters and sort: filter by `externalId`, `space`, `createdTime`, and `lastUpdatedTime`; sort by `externalId` and `space`. Align validators with Cognite `instances.query` (no `deletedTime` filter; timestamps are not sortable).
...
- f647a19: Fix `datapoints.retrieve` for Cognite request limits: never forward `limit: -1` (use 10_000 per page instead), chunk time series into groups of 100, and cursor-paginate until all pages are collected when `limit` is `-1`.

## 0.20.0

### Minor Changes

- bc209e4: Add `ViewMapperCache` so generated packages and `CogniteCoreClient` embed the data model schema at generation time. The client uses those cached views instead of fetching them from CDF at runtime. Enabled by default for the CLI; pass `--no-view-mapper-cache` to skip it.
